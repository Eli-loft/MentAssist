...
def _assert_client_access(client: dict, current_user: dict):
    if not client:
        raise HTTPException(status_code=404, detail="Client not found")
    if current_user["role"] != "admin" and client["counselor_id"] != current_user["id"]:
...
def delete_client(client_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    client = repo.get_client(db, client_id)
    _assert_client_access(client, current_user)
    repo.delete_client(db, client_id, user_id=current_user["id"])
